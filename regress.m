...
%% regression 
for (idex=1:numel(interval))
    ie=interval(idex);
    je=interval(idex+1);
    %%regeession line
    line=polyfit(ie:1:je,age(ie:je));
    error(idex)=line.error;
    y1end=line.predict(i);
    y2end=line.predict(j);
...
plot(period(interval),age(interval),'o',line,period,age,'*');
%xlabel(string);
y1=get(gca,'ylim');
hold on
[r c]=size(interval);
